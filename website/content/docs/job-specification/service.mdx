...
- `on_update` `(string: "require_healthy")` - Specifies how checks should be
  ...
  to true. `check_restart` can however specify `ignore_warnings` set to true
  with `on_update` equal to `require_healthy`. If `on_update` is set to
  `ignore`, `check_restart` must be omitted entirely.
